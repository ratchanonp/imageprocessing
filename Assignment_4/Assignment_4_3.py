...
def main(img, img_name):
    ...
    if os.path.exists(f"out/{img_name}") is False:
    ...
    # Radius
    radius_list = np.arange(10, 100, 10)

    # Cut-off D0
    D0_list = np.arange(10, 100, 10)
    ...
    # Butterworth Mask
    low_pass_butterworth_mask_list = [{
        "mask": create_butterworth_mask(width, height, D0, n),
        "title": f"Low Pass Butterworth Mask with D0 {D0} and n {n}"
    } for D0 in D0_list for n in n_list]

    high_pass_butterworth_mask_list = [{
        "mask": create_butterworth_mask(width, height, D0, n, invert=True),
        "title": f"High Pass Butterworth Mask with D0 {D0} and n {n}"
    } for D0 in D0_list for n in n_list]

    # Concatenate all masks
    masks = low_pass_mask_list + high_pass_mask_list + low_pass_gaussian_mask_list + \
        high_pass_gaussian_mask_list + low_pass_butterworth_mask_list + high_pass_butterworth_mask_list
